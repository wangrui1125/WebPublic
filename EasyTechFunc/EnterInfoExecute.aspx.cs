...
{
    public partial class EnterInfoExecute : BaseWWW
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
          ...
          //  int role=Convert .ToInt16 ( com.ExecuteScalar ().ToString ());
            ...
            if (Admin == 1)
            {
                Url = "../Tmp/MyQuery.aspx?n=myenpExecute7";
            }
            else if (Researcher == 1)
            {
                Url = "../Tmp/MyQuery.aspx?n=myenpExecute5&ID=" + CurrentUser.Id;
            }
            else if (Enterprise == 1)
            {
                Url = "../Tmp/MyQuery.aspx?n=myenpExecute3&ID=" + CurrentUser.Id;
            }

            else if (techConsult == 1)
            {
                Url = "../Tmp/MyQuery.aspx?n=myenpExecute2&ID=" + CurrentUser.Id;
            }
            else if (Manager == 1)
            {
                Url = "../Tmp/MyQuery.aspx?n=myenpExecute6&ID=" + CurrentUser.Id;
            }
            else if (Consult == 1)
            {
                Url = "../Tmp/MyQuery.aspx?n=myenpExecute4&ID=" + CurrentUser.Id;
            }
            else
            {
                return;
